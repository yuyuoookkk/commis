...
import { fetchApprovedReviews, reviewsEnabled, type Review } from "@/lib/reviews"
import ReviewModal from "@/components/ui/ReviewModal"

gsap.registerPlugin(ScrollTrigger)

// Shown until the first guest reviews are approved.
const placeholders = [
  {
    name: "Sarah Jenkins",
...
function ReviewCard({
  ...
  return (
    <figure className="test-card glass-dark p-8 rounded-2xl relative border border-white/5 hover:border-luxury-gold/20 transition-colors flex flex-col">
      ...
      <blockquote className="text-gray-300 text-base leading-relaxed mb-8 relative z-10 italic">
        &ldquo;{text}&rdquo;
      </blockquote>

      <figcaption className="flex items-center gap-4 mt-auto">
        <div className="w-12 h-12 rounded-full bg-luxury-gold/20 flex items-center justify-center font-serif text-luxury-gold text-xl border border-luxury-gold/30 shrink-0">
...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    fetchApprovedReviews()
      ...
      .catch(() => { /* fall back to the placeholders below */ })
    return () => { active = false }
  }, [])
  ...
  }, { scope: containerRef, dependencies: [reviews], revertOnUpdate: true })
  ...
  return (
    <>
          ...
          <div className="test-header text-center mb-16 max-w-2xl mx-auto">
            ...
            {reviewsEnabled && (
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-luxury-gold/30 bg-luxury-gold/5 text-luxury-gold font-semibold text-sm hover:bg-luxury-gold/10 transition-colors cursor-pointer"
              >
            ...
            {reviews.length > 0
              ? reviews.map((review) => (
                  <ReviewCard
                    key={review.id}
                    name={review.name}
                    meta={formatMonth(review.created_at)}
      ...
      {formOpen && <ReviewModal onClose={() => setFormOpen(false)} />}
    </>
  )
}
